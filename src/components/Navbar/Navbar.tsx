...
function Navbar({ openNav }: Props) {
  ...
  return (
    <div className={`fixed ${navBg ? 'bg-white shadow-lg' : 'fixed'} w-full h-[10vh] z-[1000]`}>
      <div className='flex items-center h-full justify-between w-full mx-auto px-5 xl:px-8 '>
        ...
        <div>
          <button className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full hidden md:block lg:block'>Join us</button>
          <Bars3Icon onClick={openNav} className='w-8 h-8 md:hidden lg:hidden cursor-pointer' />
        </div>
      </div>
    </div>
  )
